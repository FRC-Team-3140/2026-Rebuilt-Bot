...
public class ManualAim extends AimType {
    ...
    public void periodic(double deltaTime, double hoodMeasurement, double flywheelMeasurement,
        ...
        desiredRotationAngle += -Controller.getInstance().getRightX(controllers.SECONDARY) * manualAimRotationSpeed
                * deltaTime;
        ...
        rotationAngle = Math.max(-90,
                Math.min(desiredRotationAngle - Odometry.getInstance().getRotation().getDegrees(), 90));
        if (Math.abs(Controller.getInstance().getRightX(controllers.SECONDARY)) > 0.02
                && (desiredRotationAngle < -90 || desiredRotationAngle > 90))
            desiredRotationAngle = rotationAngle + Odometry.getInstance().getRotation().getDegrees();
